...
n_epochs = 501
writer = SummaryWriter('ex_test/cnn_result')
...
torch.save(model.state_dict(), './  CNNAE_tanh2.pth')
...
train =pd.read_csv('D:/PYTHON/外插1.csv', names=columns)
...
test_quantity = quantity[2500:2812,:]
#print(test_quantity.shape)
# (340,2)

test_one = train_one[2500:2812]
test_two = train_two[2500:2812]
test_three = train_three[2500:2812]
#print(test_one.shape)
# (340)

train_yR = train_y[0:2500,0:1]
train_yT = train_y[0:2500,1:2]
#print(len(train_y))
#2800
train_quantity = quantity[0:2500,:]

train_one = train_one[0:2500]
train_two = train_two[0:2500]
train_three = train_three[0:2500]
...
#print(len(train_y))
#2800
val_quantity = quantity[2000:2500,:]

val_one = train_one[2000:2500]
val_two = train_two[2000:2500]
val_three = train_three[2000:2500]
######
...
MSE_loss = []
#embeddingvalue = []
writer = SummaryWriter('ex_test/NN_result')
...
for epoch in range(EPOCH):
    ...
    for i in range(len(train_yR)):
        ...
        y = torch.FloatTensor(train_quantity[i]).unsqueeze(0)
...
#         print('epoch',epoch,loss.item())
    ...
    total_loss1 = 0
    total_loss2 = 0
    for i in range(len(val_yR)):
        
        ### train AE
        iteration += 1
        targetR = torch.FloatTensor(val_yR[i]).view(-1,1).detach()
        ...
        x = torch.cat((en_input1,en_input2,en_input3),0)
        y = torch.FloatTensor(val_quantity[i]).unsqueeze(0)
        f1,f2,f3,d1,d2,d3,outR = my_model(x,y)
    ...
    #print(features.shape)
    #print(type(features))
    writer.add_scalar('val_AE_loss', loss1, epoch)
    writer.add_scalar('val_MSE_loss', loss2, epoch)
    if(epoch%10 == 0):
        print('epoch [{}/{}], val_mse_loss:{:.4f}'
          .format(epoch, EPOCH, loss2))

torch.save(my_model.state_dict(), './  NNmodel.pth')
