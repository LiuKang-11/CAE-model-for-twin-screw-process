...
test_yR = train_y[0:326,0:1]
test_yT = train_y[0:326,1:2]
std2_test_yT= normalize(test_yT)
#print(len(test_y),test_y.shape)
# (340,2)

test_quantity = quantity[0:326,:]
#print(test_quantity.shape)
# (340,2)

test_one = train_one[0:326]
test_two = train_two[0:326]
test_three = train_three[0:326]
...
my_model2.load_state_dict(torch.load('D:/PYTHON/  std2_NNmodel_temp.pth'))
loss_func = nn.MSELoss()

writer = SummaryWriter('ex2_dataSTD_temp/NN_result')
for epoch in range(1):
    ...
    for i in range(len(test_yR)):
        ...
        ### train AE
        targetR = torch.FloatTensor(std2_test_yT[i]).view(-1,1).detach()
